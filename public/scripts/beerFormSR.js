...
var OnTapList = React.createClass({
    ...
    handleDelete: function(beer){
      alert("DELETE!")
      var getId = this.props.data.map(function(beer){
        return <div> {beer.id}</div>
        }
      }),
                 ...
                 <table className="table">
                   <tbody>
                     <tr className="col-md-3">
                      ...
                      <td style={{width:"10%"}}><button onClick={that.handleDelete}><i className="fa fa-minus-circle"></i></button></td>
...
var App = React.createClass({
    ...
    render: function() {
        return (
            <div>
            <OnTapList data={this.state.data} url={this.state.data}/>
            </div>
            )
    }
})
